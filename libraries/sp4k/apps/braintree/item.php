<?php
    ...
    class Sp4kAppsBraintreeItem extends Sp4kAppsAbstractItem
    {
        ...
        public function __construct(\Joomla\Registry\Registry $state)
        {
        ...
        public function process(){
            ...
            $payment = Braintree_Transaction::sale([
                'amount' => $this->getState()->get('amount',false),
                'paymentMethodNonce' => $this->getState()->get('paymentMethodNonce',false)
            ]);


            $this->_payment = $payment;

            $this->status = (int)$this->_payment->success;
            if($this->status){
                $this->transaction_id = $payment->transaction->id;
                $this->amount = $this->getState()->get('amount');
            }else{
                $this->_error = $this->_payment->message;;
            }
